            ...
            .alert("Enter a Number", isPresented: $isAdd) {
                           ...
                           Button("OK", action: {
                               if let num = Double(inputNumber) {
                                   ...
                                   print(isMealEmpty)
                                   if isMealEmpty {
                                       ScheduleViewModel().CreateSchedule(schedule: ScheduleCreateFull(user_id: 0, time: getCurrentDateFullString(), detail:[ScheduleDetailCreate(exersice_id: exersice.id, rep: Int(num), time: Int(num))] )){success in
                                           if success {
                                               alertsuccess = true
                                           }else {
                                               alertfail = true
                                       ...
                                       ScheduleViewModel().CreateScheduleDetail(schedule: ScheduleDetailCreate(exersice_id: exersice.id, rep: Int(num), time: Int(num))){success in
                                               if success {
                                                   alertsuccess = true
                                               }else {
                                                   alertfail = true
